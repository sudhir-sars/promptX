    ...
    return (
        <div className="h-full flex-1 space-y-7 overflow-y-auto no-scrollbar px-6 py-20 md:px-10 lg:px-16 transition-all duration-300 ease-in-out">
            ...
            <section className="overflow-hidden rounded-4xl border">
                <div className="flex flex-col items-center px-8 py-16 text-center">
                    ...
                    <p className="mt-4 max-w-2xl text-sm leading-6 text-muted-foreground">
                        Invite teammates, manage permissions, assign roles, and collaborate securely
            ...
            <div className="grid gap-4 md:grid-cols-3">
                ...
                <FeatureCard
                    title="Roles"
                    description="Configure owner, admin, and member permissions."
                />

                <FeatureCard
                    title="Access Control"
                    description="Control who can edit, publish, and deploy prompts."
                />
            </div>
        </div>
    );
...
function Badge({ label }: { label: string }) {
    return (
        <div className="rounded-full border px-3 py-1 text-xs text-muted-foreground">{label}</div>
    );
}
