...
/* ------------------------------------------------------------------ */
export function DocPageWrapper({ children, className }: { children: React.ReactNode; className?: string }) {
...
export function DocSection({
	...
	return (
		<section id={id} className={cn("scroll-mt-20", className)}>
			{label && <p className="mb-3 text-[10px] font-medium uppercase tracking-[0.15em] text-foreground/30">{label}</p>}
			{title && (
...
export function DocH3({ children, id, className }: { children: React.ReactNode; id?: string; className?: string }) {
	return (
		<h3 id={id} className={cn("scroll-mt-20 text-[13px] font-semibold tracking-tight text-foreground/80", className)}>
			{children}
		</h3>
	);
}
...
export function DocParagraph({ children, className }: { children: React.ReactNode; className?: string }) {
	return <p className={cn("text-[13px] leading-[1.75] text-muted-foreground", className)}>{children}</p>;
}
...
export function CodeBlock({
	...
	return (
		<div className={cn("overflow-hidden rounded-4xl border border-foreground/[0.06]", className)}>
			...
			<pre className="overflow-x-auto p-5">
				<code className="font-mono text-[12px] leading-[1.75] text-foreground/65">{code}</code>
			</pre>
		</div>
	);
}
...
export function TerminalBlock({ commands, className }: { commands: string | string[]; className?: string }) {
...
export function PropTable({
	...
	return (
		<div className={cn("overflow-x-auto rounded-2xl border border-foreground/[0.06]", className)}>
			<table className="w-full text-left">
				...
				<tbody>
					{rows.map((row, i) => (
						<tr
							key={typeof row[0] === "string" ? row[0] : `row-${i}`}
								...
								return (
									<td
										...
										className={cn(
											...
											typeof cell === "string" && !cell.includes(" ") && cell.length < 40 ? "font-mono" : "",
										)}
									>
										{cell}
									</td>
			...
			{steps.map((step, i) => (
				<div key={step.title} className="relative flex gap-5 pb-9 last:pb-0">
					...
					<div className="min-w-0 flex-1 pt-[5px]">
						...
						<div className="mt-2 text-[13px] leading-[1.7] text-muted-foreground">{step.content}</div>
					</div>
				</div>
			))}
		</div>
...
/*  DocList                                                            */
/* ------------------------------------------------------------------ */
export function DocList({ items, className }: { items: (string | React.ReactNode)[]; className?: string }) {
	return (
...
export function DocCard({
	...
	const content = (
		<>
			<p className="text-[11px] font-medium uppercase tracking-[0.12em] text-foreground/65">{title}</p>
	...
	return <div className={cn("rounded-2xl border border-foreground/[0.06] p-5", className)}>{content}</div>;
}
