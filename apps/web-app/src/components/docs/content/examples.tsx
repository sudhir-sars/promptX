    ...
    <DocPageWrapper>
      <div>
        ...
        <DocParagraph className="mt-4 max-w-lg">
          Production patterns for integrating PromptX into real applications. Each example is
          self-contained and ready to adapt.
        </DocParagraph>
      </div>
      <DocDivider />
      <DocSection id="basic-fetch" label="Pattern" title="Basic prompt fetch">
        <DocParagraph>
          The simplest integration — fetch a deployed prompt and pass it to your LLM provider. The
          SDK resolves the currently active version for the given environment automatically.
        </DocParagraph>
        <CodeBlock
          className="mt-5"
          language="typescript"
  ...
  messages: [
    ...
    { role: "user", content: userMessage },
      ...
      <DocSection id="template-variables" label="Pattern" title="Template variables">
        <DocParagraph>
          PromptX supports Mustache-style template variables. Define placeholders in your prompt and
          resolve them at fetch time.
        </DocParagraph>
        <CodeBlock
          className="mt-5"
          language="text"
...
// prompt.content is the fully resolved string`}
      ...
      <DocSection id="ab-testing" label="Pattern" title="A/B testing with traffic splits">
        <DocParagraph>
          PromptX supports percentage-based traffic splitting between deployed prompt versions. To
          ensure accurate experiment results, you must provide a stable unique identifier via
          <InlineCode>sessionId</InlineCode>. This can be a user ID, chat ID, conversation ID,
          session ID, or any identifier that uniquely represents the current user or conversation.
        </DocParagraph>

        <Callout type="warning" className="mt-4">
          A/B testing requires a stable <InlineCode>sessionId</InlineCode>. If no{" "}
          <InlineCode>sessionId</InlineCode> is provided, PromptX resolves requests to the version
          with the highest traffic allocation.
        </Callout>
...
// throughout the experiment`}
        ...
        <Callout type="note" className="mt-4">
          ...
          <InlineCode>sessionId</InlineCode> is provided, PromptX automatically performs sticky
          allocation and ensures the same user or conversation remains on the same variant.
        </Callout>
      </DocSection>
      <DocDivider />
      ...
      <DocSection id="error-handling" label="Pattern" title="Error handling and fallbacks">
        <DocParagraph>
          The SDK throws typed errors for common failure cases. Always handle errors gracefully,
...
try {
  ...
  throw error;
        ...
        />
        <Callout type="warning" className="mt-4">
          Always have a fallback prompt for critical code paths. Network issues and API outages
          happen — your application should degrade gracefully.
        </Callout>
      </DocSection>
      <DocDivider />
      ...
      <DocSection id="middleware" label="Pattern" title="Middleware and hooks">
        <DocParagraph>
          Register middleware functions to intercept, transform, or log prompt resolution. Useful
          for observability, feature flags, and dynamic overrides.
        </DocParagraph>
        <CodeBlock
          className="mt-5"
          language="typescript"
