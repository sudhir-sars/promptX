    ...
    <DocPageWrapper>
      <section className="border-b border-border/50 pb-10">
        ...
        <DocParagraph className="mt-5 max-w-2xl text-balance">
          PromptX gives you version control, instant rollbacks, and gradual rollouts for every
          prompt in production. This guide walks you through installation, project creation, and
      ...
      <DocSection id="prerequisites" label="Before you begin" title="Prerequisites">
        <DocParagraph>
          You need a PromptX account and Node.js 18+ installed. The SDK supports all major runtimes,
          but this guide uses Node.js so the setup stays concrete and easy to follow.
        </DocParagraph>
      ...
      <DocSection id="configure" label="Step 2" title="Configure your API key">
        <DocParagraph>
          ...
          <InlineCode>Settings → API Keys</InlineCode>, then store it in your environment variables.
        </DocParagraph>
...
export const promptx = new PromptX({
  ...
  projectId: process.env.PROMPTX_PROJECT_ID!,
      ...
      <DocSection id="first-prompt" label="Step 4" title="Fetch your first prompt">
        <DocParagraph>
          Once you&apos;ve created a prompt in the dashboard and deployed it, you can fetch it from
          your application code. The SDK resolves the currently active deployment automatically.
        </DocParagraph>
        <CodeBlock
          className="mt-5"
          language="typescript"
...
export async function POST(req: Request) {
  ...
  return Response.json({ reply: response.choices[0].message.content });
        ...
        <Callout type="note" className="mt-4">
          <InlineCode>promptx.get()</InlineCode> automatically resolves traffic-split deployments.
          If you have a canary deployment at 10%, 10% of calls will receive the canary version.
        </Callout>
      </DocSection>
