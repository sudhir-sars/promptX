    ...
    <DocPageWrapper>
      <div>
        ...
        <DocParagraph className="mt-4 max-w-lg">
          Move from hardcoded prompts to PromptX. This guide walks through a typical migration with
          practical examples and a verification checklist.
        </DocParagraph>
      </div>
      ...
      <DocSection id="from-hardcoded" label="Migration path" title="From hardcoded prompts">
        <DocParagraph>
          Most teams start with prompts embedded directly in application code. The migration is
          straightforward: extract each prompt into PromptX, then replace the hardcoded string with
          an SDK call.
        </DocParagraph>
  ...
  messages: [
    ...
    { role: "user", content: userMessage },
        ...
        <Callout type="tip" className="mt-4">
          Start by migrating your most frequently changed prompts first. They benefit the most from
          version control and no-deploy updates.
        </Callout>
      </DocSection>
          ...
          steps={[
            {
              ...
              content: (
                <>
                  ...
                  <InlineCode>system_prompt</InlineCode>, <InlineCode>SYSTEM_PROMPT</InlineCode>.
                  Document each one with its location and purpose.
                </>
              ),
            },
            {
