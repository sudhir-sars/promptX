...
export function useTeamMembers() {
    ...
    const memberIds = useMembersStore((state) =>
        teamId ? (state.memberIdsByTeam[teamId] ?? EMPTY_ARRAY) : EMPTY_ARRAY,
    );

    const membersById = useMembersStore((state) => state.membersById);

    const members = useMemo(
        () => memberIds.map((id) => membersById[id]).filter(Boolean),
        [memberIds, membersById],
    );

    const cursor = useMembersStore((state) =>
