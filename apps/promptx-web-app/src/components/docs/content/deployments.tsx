...
export default function DeploymentsContent() {
        ...
        <DocPageWrapper>
            <div>
                ...
                <DocParagraph className="mt-4 max-w-lg">
                    Ship prompt changes to production without touching your codebase. Gradual
                    rollouts, traffic splits, canary releases, and instant rollback — all from the
                    dashboard or API.
                </DocParagraph>
            </div>
            ...
            <DocSection id="deployment-model" label="Concepts" title="The deployment model">
                <DocParagraph>
                    ...
                    <InlineCode>promptx.get("checkout-assistant")</InlineCode>. You can deploy a
                    single version, run an A/B test between multiple versions, or allocate traffic
                    across several versions to control rollout behavior.
                </DocParagraph>
            </DocSection>
            <DocDivider />
            ...
            <DocSection id="traffic-splitting" label="Traffic" title="Traffic splitting">
                <DocParagraph>
                    When deploying a new version while another version is already deployed, you can
                    either replace the existing deployment or create an A/B test. A/B tests
                    distribute traffic across multiple versions according to the traffic allocations
                ...
                <DocParagraph className="mt-2">
                    A/B testing requires a <InlineCode>sessionId</InlineCode>. The value can be any
                    stable unique identifier that represents the current user or session, such as a
                    chat ID, conversation ID, session ID, or user ID. PromptX uses this identifier
                    to consistently assign requests to the same version based on the deployment's
  ...
  sessionId: chat.id,
                ...
                <Callout type="warning" className="mt-4">
                    If an A/B test deployment is configured and no{" "}
                    <InlineCode>sessionId</InlineCode> is provided, PromptX serves the version with
                    the highest configured traffic allocation.
                </Callout>
            </DocSection>
            <DocDivider />
            ...
            <DocSection id="rollback" label="Recovery" title="Rollback">
                <DocParagraph>
                    Rollback updates the active deployment to serve a previously deployed version.
                    No new version is created and your version history remains unchanged.
                </DocParagraph>

                <DocH3 className="mt-5">From the dashboard</DocH3>

                <DocParagraph className="mt-2">
                    Open the active deployment and click <InlineCode>Rollback</InlineCode>. You'll
                    be asked to confirm the target version before the deployment is updated.
                </DocParagraph>

                <Callout type="note" className="mt-4">
                    Rollbacks typically take effect within seconds. During this brief propagation
                    period, some requests may continue to receive the previous deployment
                    configuration.
                </Callout>
            </DocSection>
